...
const int MN = 1100;
int N,M;
int SN[MN], W[MN], Time[MN][MN];
vector< pair<int,int> > e[MN*MN];
int getid(int x, int y){
    return x*MN + y;
}
bool in[MN*MN];
int dist[MN*MN];
int Mindist(int S, int T){
    queue<int> q;
    RST(in);
    FLC(dist, inf);
    ...
    if ( inf==dist[T] ) return -1;
    else return dist[T];
}
void run(){
    for (int i=0; i<MN*MN; i++) e[i].clear();

    cin>>N;
    for (int i=1; i<=N; i++){
        cin>>SN[i]>>W[i];
        for (int j=1; j<=SN[i]-1; j++){
            cin>>Time[i][j];
        }
        for (int a=1; a<=SN[i]; a++){
            int t = W[i];
            for (int b=a+1; b<=SN[i]; b++){
                t += Time[i][b-1];
                e[ getid(i,a) ].push_back( make_pair( getid(i,b), t ) );
                e[ getid(i,b) ].push_back( make_pair( getid(i,a), t ) );
                //cerr<<getid(i,a)<<" "<<getid(i,b)<<endl;
            }
        }
    }
    //tunnel
    ...
    for (int i=0; i<M; i++){
        int m1, s1, m2, s2, t;
        cin>>m1>>s1>>m2>>s2>>t;
        e[ getid(m1,s1) ].push_back( make_pair( getid(m2,s2), t ) );
        e[ getid(m2,s2) ].push_back( make_pair( getid(m1,s1), t ) );
        //cerr<<getid(m1,s1)<<" "<<getid(x2,y2)<<endl;
    }
    ...
    while(Q--){
        int x1,y1,x2,y2;
        cin>>x1>>y1>>x2>>y2;
        cout<<Mindist( getid(x1,y1), getid(x2,y2) )<<endl;

        //cerr<<getid(x1,y1)<<" "<<getid(x2,y2)<<endl;
    }
}
int main(){
