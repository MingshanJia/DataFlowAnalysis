...
bool vis[MN][MN];
int N;
bool iszero(int ox, int oy){
    if ( vis[ox][oy] ) return false;
    if ( '*'==g[ox][oy] ) return false;
    for (int c=0; c<8; c++){
        int x = ox+dx8[c];
        int y = oy+dy8[c];
...
void click(int ox, int oy){
    if ( !iszero(ox, oy) ) return ;
    vis[ox][oy] = true;
    for (int c=0; c<8; c++){
        int x = ox+dx8[c];
        int y = oy+dy8[c];
