...
int R,C;
string m[20];
int t[15*15*(15*15+1)*(15*15+1)];
void add(int x,int y,int x0,int y0,int x1,int y1,int dist,vi &q){
  int code=((x*C+y)*(15*15+1)+(x0>=0&&y0>=0?x0*C+y0:15*15))*(15*15+1)+(x1>=0&&y1>=0?x1*C+y1:15*15);
  if(t[code]==-1){
    t[code]=dist;
    q.push_back(code);
  }
  ...
  for(int c=1;c<=z;c++){
    ...
    while(q2.size()){
      ...
      FOR(i,q.size()){
        ...
        if(b<15*15){x0=b/C;y0=b%C;}else x0=y0=-1;
        if(c<15*15){x1=c/C;y1=c%C;}else x1=y1=-1;
