...
long double price_of_gas;
int perish[15];
long double dst[60][60];
int cost[60][15];
int x[60];
int y[60];

long double memo[1 << 15][60][2];
double solve(int st, int msk, int exp)
{
	if(msk == (1 << items) - 1) return price_of_gas * dst[st][0];
	long double & ref = memo[msk][st][exp];
	if(ref != 0) return ref;
...
int main()
{
	...
	{cout << k << endl;
		...
		fout.precision(7);
		fout << "Case #" << k << ": " << fixed << solve(0, 0, 0) << endl;
		//fout << out << endl;
	}
