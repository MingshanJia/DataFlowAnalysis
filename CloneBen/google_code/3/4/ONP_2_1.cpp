...
void check(TP a)
{
	int flag = 1;
	for (int i = 0; i < 3; i++)
		if (!(dis(circle[i].o, a) < circle[i].r + eps))
			flag = 0;
	if (flag) gflag = 1;
}
...
int main()
{
	...
	for (scanf("%d", &T); T; T--) {
		...
		double dot = (x[1] - x[0]) * (x[2] - x[0]) + (y[1] - y[0]) * (y[2] - y[0]) + (z[1] - z[0]) * (z[2] - z[0]);
	...
	//	cerr << ag3 << endl;
		...
		while (!(fabs(dis(p[0], p[1]) - l1) < eps && fabs(dis(p[0], p[2]) - l2) < eps && fabs(dis(p[1], p[2]) - l3) < eps))
			cerr << "?" << dis(p[0], p[1]) << ' ' << l1 << ' '<< dis(p[0], p[2]) << ' ' << l2 <<  ' ' << dis(p[1], p[2]) << ' ' << l3 << endl; 
		double l = 0, r = 10000;
