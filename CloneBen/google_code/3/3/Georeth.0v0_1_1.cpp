...
void solve() {
    ...
    map<int64_t, int> sum;
    {
        ...
        for (auto kv : m) {
            vec.push_back(make_pair(1LL * kv.first, kv.second));
        }
        int s = 0;
        for (int i = 0; i < vec.size(); ++i) { // bottom
            s += C(vec[i].second, 2);
            sum[vec[i].first] = s;
        }
    }
    int64_t ans = 0;
    for (int i = 0; i < vec.size(); ++i) { // bottom
        int64_t x = vec[i].first;
        for (int j = 0; j < i; ++j) { // top
            int64_t y = vec[j].first;
            int64_t cnt = prev(sum.end())->second;
            auto it = sum.lower_bound((x - y) / 2);
            if (it->first * 2 + y > x) {
                if (it == sum.begin())
                    ;
                else {
            ...
            cnt -= C(vec[i].second, 2);
            if (y * 3 > x)
                cnt -= C(vec[j].second, 2);
            ans += vec[i].second * vec[j].second * cnt;
            ans += vec[j].second * C(vec[i].second, 3); // k == i
            if (y * 3 > x)
                ans += vec[i].second * C(vec[j].second, 3);
        }
    }
    printf("%lld\n", ans);
}
