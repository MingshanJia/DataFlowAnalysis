...
{
	int R=0;
	for (int set=0;set<two(n*m);set++)
	{
		int d[10][10];
		memset(d,0,sizeof(d));
		for (int mask=set,i=0;i<n;i++) for (int j=0;j<m;j++,mask/=2)
		{
			int x=i,y=j;
			if (s[i][j]=='-') if (mask&1) y--; else y++;
			if (s[i][j]=='|') if (mask&1) x--; else x++;
			if (s[i][j]=='\\') if (mask&1) x--,y--; else x++,y++;
			if (s[i][j]=='/') if (mask&1) x--,y++; else x++,y--;
			x=(x%n+n)%n;
			y=(y%m+m)%m;
			d[x][y]++;
		...
		bool good=true;
		for (int i=0;i<n;i++) for (int j=0;j<m;j++) if (d[i][j]>1) good=false;
		if (good) R++;
	}
	printf("%d\n",R);
}
