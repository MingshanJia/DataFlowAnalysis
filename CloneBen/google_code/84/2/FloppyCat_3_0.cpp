...
static unsigned long long digitCounts_zero[128][128][4096];
...
static void init(unsigned int B) {
	...
	/*
	for (unsigned int maxDigit=0; maxDigit<=B; maxDigit++) {
		fill(digitCounts_zero[0][maxDigit], digitCounts_zero[0][maxDigit]+maxSum+1, 0);
		digitCounts_zero[0][maxDigit][0] = 1;
	}
	for (unsigned int digitCount=1; digitCount<=B; digitCount++) {
		for (unsigned int maxDigit=0; maxDigit<=B; maxDigit++) {
			for (unsigned int sum=digitCount*(digitCount-1)/2; sum<=maxSum && sum<=digitCount*B; sum++) {
				unsigned long long res = 0;
				for (unsigned int lastDigit=0; lastDigit<maxDigit && lastDigit<=sum; lastDigit++)
					res += digitCounts_zero[digitCount-1][lastDigit][sum-lastDigit];
				digitCounts_zero[digitCount][maxDigit][sum] = res % M;
			}
		}
	}
	*/
