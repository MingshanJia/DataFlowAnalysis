...
//	freopen("..\\C-large.in","r",stdin);freopen("..\\C-large.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		{
			for (int i=1;i<=sizeX;i++) for (int j=1;j<=sizeY;j++)
				if (contain(set,ID(i,j)-1))
					A[i][j]=1;
				else
					A[i][j]=0;
			int total=0;
			for (int j=1;j<=sizeY;j++) total+=A[(sizeX+1)/2][j];
			if (total<=result) continue;
			bool good=true;
			for (int i=1;good && i<=sizeX;i++) for (int j=1;good && j<=sizeY;j++)
			{
				int E=0;
				for (int dx=-1;dx<=1;dx++) for (int dy=-1;dy<=1;dy++)
				{
					int x=i+dx,y=j+dy;
					if (x>=1 && x<=sizeX && y>=1 && y<=sizeY) E+=A[x][y];
				}
				if (E!=G[i][j]) good=false;
			}
			if (!good) continue;
