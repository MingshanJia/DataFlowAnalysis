...
int p[1000],first[1000],next[1000],v[1000];
bool dfs(int u,int f,int q){
    p[u]=q;
    for(int i=first[u];~i;i=next[i]){
        int c=v[i];
        if(c!=f){
            if(p[c]==-1){
                if(!dfs(c,u,!q))return false;
            }
            else if(p[c]!=!q)return false;
        }
    }
    return true;
}
...
int main(){
    ...
    while(t--){
        ...
        for(int i=0;i<n;i++){
            scanf("%s%s",a,b);
            if(mapa.find(a)==mapa.end())mapa[a]=l++;
            int u=mapa[a];
            if(mapa.find(b)==mapa.end())mapa[b]=l++;
            int v=mapa[b];
            addedge(u,v);
            addedge(v,u);
        }
