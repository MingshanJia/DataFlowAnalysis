...
int ltrans;
int ans[maxn], mp[maxn][maxn], q[maxn], lq, rq;
...
int main(){
	...
	for(int i1 = 0; i1 < ncase; i1++) {
		...
		trans.clear(); ltrans = 0;
		cin >> m;
		for(int i = 0; i < m; i++) {
			string a, b;
			cin >> a >> b;
			int ida = getid(a), idb = getid(b);
			mp[ida][idb] = mp[idb][ida] = 1;
		}

		bool f = 1;
		for(int i = 0; i < ltrans; i++) if(ans[i] == 0){
			ans[i] = 1;
			lq = rq = 0;
			q[rq++] = i;
			for(; lq != rq; lq++) {
				for(int j = 0; j < ltrans; j++) if(mp[q[lq]][j]){
					if(ans[j] == 0) {
						ans[j] = 3 - ans[q[lq]];
						q[rq++] = j;
					} else if(ans[j] != 3 - ans[q[lq]])
						f = 0;
				}
			}
		}
