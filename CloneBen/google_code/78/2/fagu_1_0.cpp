...
int main() {
	...
	for (int test = 0; test < T; test++) {
		...
		/*for (int x = 0; x < X; x++) {
			sssum[x][0] = 0;
			for (int y = 0; y < Y; y++)
				sssum[x][y+1] = sssum[x][y]+y*feld[x][y];
		}*/
		int maxK = 0;
		for (int x1 = 0; x1 < X; x1++) {
			for (int x2 = x1+2; x2 < X; x2++) {
				...
				int aktw = 0;
				for (int y = 0; y < K; y++) {
					//akt += 2*(ssum[x2+1][y]-ssum[x1][y])-(x1+x2)*(sum[x2+1][y]-sum[x1][y]);
				...
				for (int y1 = 0; y1+K-1 < Y; y1++) {
					int y2 = y1+K-1;
					//if (akt - 2*(x1*(feld[x1][y1]+feld[x1][y2]) + x2*(feld[x2][y1]+feld[x2][y2])) 
					//	+ (x1+x2)*(feld[x1][y1]+feld[x1][y2]+feld[x2][y1]+feld[x2][y2]) == 0) {
					...
					if (2*raktx == (x1+x2)*raktw && 2*rakty == (y1+y2)*raktw)
						maxK = max(maxK, K);
					//akt += 2*(ssum[x2+1][y2+1]-ssum[x1][y2+1] - ssum[x2+1][y1]+ssum[x1][y1]);
					//akt -= (x1+x2)*(sum[x2+1][y2+1]-sum[x1][y2+1] - sum[x2+1][y1]+sum[x1][y1]);
