...
RealCase::RealCase()
{
    if (primes.empty()) GENPRIMES(primes, 1000000);
    cin >> A >> B >> P;
}

void factorize(LL i, vector<LL> &vi)
{
    FORIT(it, primes) {
        LL p = *it;
        if (p*p > i) break;
    ...
    if (i > 1) vi.push_back(i);
}

bool common(const vector<LL> &v, const vector<LL> &w, LL P)
{
    IT(v) i = v.begin();
    IT(w) j = w.begin();
    while (i != v.end() && *i < P) ++i;
...
{
    vector<vector<LL> > factors(B + 1);
    FORE(i, A, B) factorize(i, factors[i]);

    UnionFind uf(B + 1);
    FORE(i, A, B) FORE(j, i + 1, B) if (uf.find(i) != uf.find(j)) {
        if (common(factors[i], factors[j], P)) uf.combine(i, j);
    }

    LL res = 0;
    FORE(i, A, B) if (uf.find(i) == i) ++res;
    result.set(STR(res));
}
